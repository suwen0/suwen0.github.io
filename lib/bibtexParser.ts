import { Publication, PublicationCategory, PublicationType, ResearchArea } from '@/types/publication';
...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    const yearLabel = tags.year?.trim();
    const year = parseInt(yearLabel || '') || new Date().getFullYear();
    ...
    const category = normalizePublicationCategory(tags.category, type);
    ...
    const publication: Publication = {
      ...
      category,
      displayOrder: tags.display_order ? Number(tags.display_order) : undefined,
      selected,
      preview,
...
function normalizePublicationCategory(value: string | undefined, type: PublicationType): PublicationCategory {
  const normalized = value?.trim().toLowerCase();
  if (
    normalized === 'publication' ||
    normalized === 'working-paper' ||
  ...
  return type === 'journal' ? 'publication' : 'working-paper';
}
